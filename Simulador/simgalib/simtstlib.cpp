/*
 * FILE: simtstlib_test_fns.cpp, v.1.6.0, 3/18/2011
...
/*
double simtstlib::quad_test_fn(int num_vars, int bits_per_var, double range_low_1, double range_high_1, double range_low_2, double range_high_2 ,vector<int> genes)
{
    ...
    for(int k=0; k<num_vars; k++) {
        ...
        {
            uint_genes += (genes[k][i] * one_to_shift );
    ...
    result=quattro_test_fn_real(num_vars, x);
...
double simtstlib::quad_test_fn_real(int num_vars, double x[])
{
    double rslt=0.0;
    rslt=do_sim(x);
    return rslt;
